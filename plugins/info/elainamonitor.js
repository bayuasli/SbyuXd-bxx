...
export default {
  ...
  run: async (conn, m, { Func }) => {
    try {
      m.reply("📊 Generating Elaina Monitor Dashboard...")
      ...
      ctx.fillStyle = '#ffffff'
      ctx.textAlign = 'center'
      ctx.fillText('Elaina MONITOR', width/2, 80)
      ...
      await conn.sendMessage(m.chat, {
        image: buffer,
        caption: `📊 *Elaina Monitor Dashboard*\nGenerated in ${latencyMs.toFixed(2)}ms\n\nUse .elaina or .monitor to refresh`
      }, { quoted: m })

    } catch (err) {
      console.error('[ELAINAMONITOR ERROR]', err)
