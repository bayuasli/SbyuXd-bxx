...
export default async function useSqliteAuth(folder = './sessions') {
  ...
  const stmtGetKey = db.prepare(`SELECT data FROM keys WHERE category=? AND id=?`)
  const stmtSetKey = db.prepare(`
    INSERT OR REPLACE INTO keys (category, id, data, updated_at)
    VALUES (?, ?, ?, ?)
  `)
  const stmtDelKey = db.prepare(`DELETE FROM keys WHERE category=? AND id=?`)
